...
function initVoiceRecognition() {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
        ...
        recognition.onstart = function() {
            ...
            statusEl.textContent = "🎤 Слушаю... Говорите сейчас";
        ...
        recognition.onresult = function(event) {
            ...
            statusEl.textContent = "✓ Сообщение распознано";
        ...
        recognition.onerror = function(event) {
            ...
            voiceBtn.style.background = "linear-gradient(135deg, #0056b3, #003d82)";
            
            switch(event.error) {
                case 'not-allowed':
                case 'permission-denied':
                    statusEl.textContent = "❌ Разрешите доступ к микрофону";
                    break;
                case 'no-speech':
                    statusEl.textContent = "❌ Речь не распознана";
                    break;
                case 'audio-capture':
                    statusEl.textContent = "❌ Микрофон не найден";
                    break;
                case 'network':
        ...
        console.log("Голосовой ввод не поддерживается браузером");
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if('webkitSpeechRecognition' in window || 'SpeechRecognition' in window){
        ...
        recognition.continuous = false;
        recognition.interimResults = false;
        recognition.lang = 'ru-RU'; // Можно менять на 'en-US' или 'kk-KZ'
        
        recognition.onresult = function(event){
            const transcript = event.results[0][0].transcript.trim();
            textIn.value = transcript;
            sendBtn.click(); // Отправляем распознанный текст как сообщение
        };
